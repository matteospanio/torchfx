...
import numpy as np
import torch
from scipy.signal import butter, iirpeak
...
class HighPass(IIR):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        b, a = butter(self.order, self.cutoff / (0.5 * self.fs), btype="high")
        return F.lfilter(
            x,
...
class LowPass(IIR):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.fs is None:
            raise ValueError(NONE_FS_ERR)
        b, a = butter(self.order, self.cutoff / (0.5 * self.fs), btype="low")
        return F.lfilter(
            x,
            torch.as_tensor(a, dtype=dtype, device=device),
            torch.as_tensor(b, dtype=dtype, device=device),
